...
class Backorder extends Base {
	...
	public function __settings ( $settings ) {
        $settings['sections'][ $this->id ] = [
            ...
            'fields'	=> [
                [
                    'id'      	=> 'backorder-default-date',
                    'label'     => __( 'BackOrder Default Date', 'codesigner' ),
                    'type'      => 'date',
                    'desc'   	=> __( 'Select if you want to select the default date of the backorder in product.', 'codesigner' )
                ],
                [
                    'id'      	=> 'backorder-default-text',
                    'label'     => __( 'BackOrder Default text', 'codesigner' ),
                    'type'      => 'text',
                    'default'   => 'Expected availability: %%available_date%%',
                    'desc'   	=> __( 'Select if you want to set the default backorder text for shop widgets.', 'codesigner' )
                ]
            ]
        ];
    ...
    public function settings_saved( $option_name, $posted_data ) {
        if ( $option_name == 'codesigner_backorder' ) {
            ...
            if ( $products ) {
                foreach ( $products as $id => $name ) {
                    ...
                    $cd_backorder_time  = get_post_meta( $id, 'cd_backorder_time', true );
                    $deafult_date       = $posted_data['backorder-default-date'];
                    
                    if ( empty( $cd_backorder_time ) && $deafult_date ) {
                        update_post_meta( $id, 'cd_backorder_time', $deafult_date );
                    }
                }
            } 
        }

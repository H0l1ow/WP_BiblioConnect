        ...
        foreach ( $product_taxonomy as $_taxonomy => $value ) {
			...
			add_filter( "manage_{$_taxonomy}_custom_column", function( $columns, $column, $term_id  ) {
				...
				if ( 'color' == $term_type ) {
					...
					printf('<div class="swatches_thumb swatch_color" ><input type="color" id="codesigner-vs-color" disabled name="codesigner-vs-color" value="%1s"></div>', $term_data );
				}				
				...
				if ( 'label' == $term_type ) {
					...
					printf("<div class='swatches_thumb swatch_color' >%s</div>", $term_data );
				}
            ...
            add_action( "{$_taxonomy}_add_form_fields", function( $taxonomy ) {	
                ...
                <div class="form-field term-color-wrap"> 
					...
					<div id="codesigner-taxonomy_text" class="codesigner-taxonomy-hidden" >
						<label><?php _e( 'Enter Your Label', 'codesigner' ) ?></label>
					...
					if ( 'color' == $attribute->type ) {
						...
						<div id="codesigner-taxonomy_color" class="codesigner-taxonomy-hidden">
							<label for="tag-color"><b><?php _e( 'Color', 'codesigner' ) ?></b></label>
							<input type="color" id="codesigner-vs-color" name="codesigner-vs-color" value="#ff0000">
							<p> 
								<?php _e( 'Input color if you want to use this as color taxonomy for CoDesigner variation swatches module Widget', 'codesigner' ) ?>							
							</p>
						</div>
						<?php
					};
					...
					<div id="codesigner-taxonomy_image" class="codesigner-taxonomy-hidden" >
						<table class="form-table">
							<tr>
								<th>
									<button id="codesigner-texonomy-upload" class="button"><?php _e( 'Upload Image', 'codesigner' ) ?></button>
								</th>
								<td>
            ...
            add_action( "{$_taxonomy}_edit_form", function( $tag, $taxonomy ) {
				...
				?>
				<div class="form-field term-color-wrap">
					<label for="tag-color" style="display: inline-block;width: 215px;"><b><?php _e( 'Color', 'codesigner' ) ?></b></label>
					<input type="color" id="codesigner-vs-color" name="codesigner-vs-color" value="<?php esc_attr_e( $term_data ); ?>">
					<p style="margin-left: 27%;"> <?php esc_attr_e( $term_data ) ?> 
						<?php _e( 'Input color if you want to use this as color taxonomy for CoDesigner variation swatches module Widget', 'codesigner' ) ?>
					</p>
				</div>
				<?php 
				...
				<table class="form-table">
					<div id="codesigner-taxonomy_image" class="codesigner-taxonomy" >
						<tr>
							<th>
								<button id="codesigner-texonomy-update" class="button"><?php _e( 'Update Image', 'codesigner' ) ?></button>
							</th>
							<td>
				...
				if ( 'label' == $attribute->type ) {
					...
					<table class="form-table">
						<tr>
							<th><?php _e( 'Label', 'codesigner' ); ?></th>
							<td><input type="text" name="codesigner-taxonomy-label" value="<?php esc_attr_e( $term_data ); ?>" ></td>
						</tr>
					</table>
					<?php
				}

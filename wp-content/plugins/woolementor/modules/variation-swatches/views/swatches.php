...
?>
<div class="cd-variation-swatches-wrapper" data-product-id='<?php esc_attr_e( $product_id ); ?>' data-variations='<?php echo json_encode( $variation_data ); ?>' >
    <?php
    ...
    <form class="cd-variation-swatches-form">
        ...
        foreach ( $used_terms as $attr_name => $values ) {
            ...
            echo '<p class="codesigner-tax-name">' . ucfirst( esc_html( $name ) ) . '</p>';
            ...
            foreach ( $values as $term_name ) {
                ...
                if( ! empty( $terms_values[$term_name]['value'] ) ) {
                    ...
                    printf( __('%1s not configured', 'codesigner'), $term_name );
                }
        ...
        ?>
        <input type="hidden" name="cd-vs-id" class="cd-vs-id" value = "0" data-product-id="<?php esc_attr_e( $product_id ); ?>" >
        <div class='cd-vs-message' data-product-id="<?php esc_attr_e( $product_id ); ?>"></div>
    </form>

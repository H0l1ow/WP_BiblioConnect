...
class Product_Brands extends Base {
	...
	public function add_custom_taxonomy_to_product_meta() {
		...
		if ( $terms ) {			
			...
			_e( Helper::get_option( 'codesigner_product_brands', 'pb_singular_name' ) .' : '  , 'codesigner' );

			foreach ( $terms as $key => $term ) {
				$term_links[ get_term_link( $term, Helper::get_option( 'codesigner_product_brands', 'pb_singular_name' ) ) ]  = $term->name;				
			}

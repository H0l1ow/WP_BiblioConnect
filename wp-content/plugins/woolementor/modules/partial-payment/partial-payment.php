...
class Partial_Payment extends Base {
    ...
    public function display_payment_option() {
        ...
        if ( get_post_meta( $product_id, 'cd_enable_partial_payment', true ) && $product->is_in_stock() ) {
            ...
            <div class="cd_partial_payment_block">
                <p><?php echo __( 'Select Payment Type', 'codesigner' ); ?></p>

                <div class="cd_radio_block">
                    <input type="radio" id="cd_full" name="cd_partial_payment_type" value="full" checked>
                    <label for="cd_full"><?php echo __( 'Full Payment', 'codesigner' ); ?></label><br>
                </div>
                
                <div class="cd_radio_block">
                    <input type="radio" id="cd_partial" name="cd_partial_payment_type" value="<?php esc_attr_e( $partial_amount_type ); ?>" >
                    <label for="cd_partial"><?php echo __( $select_field_text, 'codesigner' ); ?></label>
                </div>
                ...
                <div class="cd_partial_payment_amount_block" style="display: none;">
                    <p class="cd_partial_amount" style="display: none;">
                        ...
                        if ( $partial_amount_type === 'percentage' ) {
                            ...
                            echo sprintf( __( 'First installment( %1s%2s ): %0.2f%4s Per Item.'), $percentage_payment_amount, '&percnt;', $first_installment, get_woocommerce_currency_symbol() );
                        }
                        if ( $partial_amount_type === 'fixed' ) {
                            echo sprintf( __( 'First installment( Fixed ): %0.2f%2s Per Item.'), $fixed_payment_amount, get_woocommerce_currency_symbol() );
                        }
                        ?>
                    </p>
                    <p class="cd_custom_amount" style="display: none;">
                        <input 
                            ...
                            min         = "<?php esc_attr_e( $minimum_payment_amount ); ?>" 
                            value       = "<?php esc_attr_e( $minimum_payment_amount ); ?>"
                            placeholder = "<?php echo sprintf( __( 'Minimum Amount is %0.2f%2s'), $minimum_payment_amount, get_woocommerce_currency_symbol() ); ?>" 
                        >
                    </p>
                </div>
            </div>
    ...
    public function add_email_due_payment_link( $order ) {
        ...
        if ( $is_partial_order && $order_due > 0 ) {
            ...
            echo '<p>';
            echo __( 'Pay Due Amount: ', 'codesigner' );
            echo '<a href="' . esc_url( $due_payment_url ) . '">';
            echo __( 'Pay Now', 'codesigner' );
            echo '</a></p>';
        }
    }
    ...
    public function add_partial_payment_data( $order_id ) {
        ...
        <tr>
            ...
            <td width="1%"></td>
            <td class="total"><?php echo wc_price( $order_total ) ?></td>
        </tr>
        <tr>
            <td class="label"><?php echo esc_html__( 'Currently Paid:', 'codesigner' ); ?></td>
            <td width="1%"></td>
            <td class="total"><?php echo wc_price( $amount_paid ) ?></td>
        </tr>
        <tr>
            <td class="label"><?php echo esc_html__( 'Due:', 'codesigner' ); ?></td>
            <td width="1%"></td>
            <td class="total"><?php echo wc_price( $amount_due ) ?></td>
        </tr>
        <?php
        endif;
    }

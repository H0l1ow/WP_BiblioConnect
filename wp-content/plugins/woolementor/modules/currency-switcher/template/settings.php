    ...
    <div class="cd-cs-wrapper">
        <?php foreach ( $cd_currency as $index => $values ) { ?>
            <div class="cd-single-currency">
                <select name="cd_currency[<?php esc_attr_e( $index ) ?>][cd_cs_name]" class="cd-currency-name">
                    <?php
                    if ( function_exists( 'get_woocommerce_currencies' ) ) {
                        ...
                        foreach ( $currency_options as $currency_code => $currency_name ) {
                            ...
                            echo '<option value="' . esc_attr( $currency_code ) . '" ' . $selected . '>' . esc_html( $currency_name ) . '</option>';
                        }
                    }
                    ?>
                </select>
                ...
                if ( isset( $values['cd_cs_rate'] ) ) {
                    ?>
                    <input type="text" name="cd_currency[<?php esc_attr_e( $index ) ?>][cd_cs_rate]" value="<?php echo esc_attr( $values['cd_cs_rate'] ); ?>" placeholder="Rate" />
                    <?php
                }
                else{
                    ?>
                    <input type="text" name="cd_currency[<?php esc_attr_e( $index ) ?>][cd_cs_rate]" value="" placeholder="Rate" />
                    <?php
                }
                if ( isset( $values['cd_cs_img'] ) ) {
                    ?>
                    <input type="hidden" name="cd_currency[<?php esc_attr_e( $index ) ?>][cd_cs_img]" value="<?php echo esc_attr( $values['cd_cs_img'] ); ?>" class= 'cd-cs-image' />
                    <?php
                }
                else{
                    ?>
                    <input type="hidden" name="cd_currency[<?php esc_attr_e( $index ) ?>][cd_cs_img]" value="" class= 'cd-cs-image' />
                    <?php
                }
                ?>
                ...
                <div class="cd-cs-preview-image">
                    ...
                    <?php else : ?>
                        <button class='cd-cs-upload-image cd-cs-button'><?php _e( 'Upload Flag', 'codesigner' ); ?></button>
                    <?php endif ?>
                </div>

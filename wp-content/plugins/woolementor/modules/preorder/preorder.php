...
class Preorder extends Base {
	...
	public function __settings ( $settings ) {
        $settings['sections'][ $this->id ] = [
            ...
            'fields'	=> [
                [
                    'id'      	=> 'preorder-default-date',
                    'label'     => __( 'PreOrder Default Date', 'codesigner' ),
                    'type'      => 'date',
                    'desc'   	=> __( 'Select if you want to select the default date of the PreOrder in product.', 'codesigner' )
                ],
                [
                    'id'      	=> 'preorder-default-text',
                    'label'     => __( 'PreOrder Default text', 'codesigner' ),
                    'type'      => 'text',
                    'default'   => 'Expected availability: %%available_date%%',
                    'desc'   	=> __( 'Select if you want to set the default PreOrder text for shop widgets.', 'codesigner' )
                ]
            ]
        ];
    ...
    public function settings_saved( $option_name, $posted_data ) {
        if ( $option_name == 'codesigner_preorder' ) {
            ...
            if ( $products ) {
                foreach ( $products as $id => $name ) {
                    ...
                    $cd_preorder_time  = get_post_meta( $id, 'cd_preorder_time', true );
                    $deafult_date      = $posted_data['preorder-default-date'];
                    
                    if ( empty( $cd_preorder_time ) && $deafult_date ) {
                        update_post_meta( $id, 'cd_preorder_time', $deafult_date );
                    }
                }
            } 
        }

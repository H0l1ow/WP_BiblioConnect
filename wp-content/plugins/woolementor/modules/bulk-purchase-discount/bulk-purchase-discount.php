...
class Bulk_Purchase_Discount extends Base {
	...
	public function simple_product_content() {
		...
		<div class="cd-bpd-wrapper">
			<h4>
				<?php _e( "CoDesigner bulk Purchase Discount Rules", 'codesigner' ); ?>
			</h4>
			<?php foreach ( $cd_bpd_rules as $index => $values ) { ?>
				<div class="cd-single-discount-rule">
					<input type="text" name="cd_bpd_rules[<?php echo $index ?>][cd_bpd_quantatity]" value="<?php echo esc_attr( $values['cd_bpd_quantatity'] ); ?>" placeholder="<?php esc_attr_e( 'Quantity', 'codesigner' ); ?>" />
					<input type="text" name="cd_bpd_rules[<?php echo $index ?>][cd_bpd_amount]" value="<?php echo esc_attr( $values['cd_bpd_amount'] ); ?>" placeholder="<?php esc_attr_e( 'Discount Amount', 'codesigner' ); ?>" />
	...
	public function variable_product_content( $loop, $variation_data, $variation ) {
		...
		echo '<h4>' . __( "CoDesigner bulk Purchase Discount Rules", 'codesigner' ) . '</h4>';
	...
	public function add_discount_text() {
		...
		if ( $product->get_meta( 'cd_bpd_rules' ) ) {
			...
			printf("<tr style='background-color: #118ab2; color: #fff;'><th>%s</th><th>%s</th></tr>", __( 'Quantity', 'codesigner'), __( 'Discount','codesigner' ) );

			foreach ( $rules as $key => $rule ) {
				$quantatity	= $rule['cd_bpd_quantatity'];
				$amount		= $rule['cd_bpd_amount'];
				printf( "<tr style='text-align: center' class='col-item'><td>%s</td><td>$%s</td></tr>", $quantatity, $amount );		
			}
			echo "</table>";
		}
	}

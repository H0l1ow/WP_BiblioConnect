		...
		<div class="wl-ssl-slider wl-ssl-slick-<?php echo $section_id; ?>">
			...
			if( $products->have_posts()) : 
				while( $products->have_posts()) : $products->the_post();
					...
					?>
					   <div class="wl-ssl-single-product product <?php echo esc_attr( $alignment ); ?>">
							 ...
							 <?php if ( 'yes' == $sale_ribbon_show_hide && $product->is_on_sale() ): ?>
								<div class="wl-ssl-corner-ribbon">
									<?php
	                                    ...
	                                    esc_html( codesigner_get_sale_text_with_discount_percentage( $product, $settings['sale_ribbon_text' ] ) )
	                                );
									?>
								</div>
							 <?php endif; ?>
								...
								<?php endif; ?>
							 	...
							 	<div class="wl-ssl-product-details">
									...
									<?php do_action( 'codesigner_shop_before_flash_sale', $settings, $product ); ?>
									<div class="wl-ssl-info-icons">
										<div class="wl-ssl-price">
											<h2 class="price"><?php echo wp_kses_post( $product->get_price_html() ); ?></h2>
										</div>
										...
										if ( 'yes' == $cart_show_hide ):
											...
											<?php else: ?>
												<div class="wl-ssl-product-cart">
													<a href="<?php echo get_permalink( $product_id ); ?>" data-quantity="1" class="product_type_<?php echo esc_attr( $product->get_type() ); ?>" data-product_id="<?php esc_attr_e( $product_id ); ?>" ><i class="<?php echo esc_attr( $cart_icon['value'] ); ?>"></i></a>
												</div>
											<?php endif;
	                                    endif; 

...
class Library extends Base {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'codesigner-library', plugins_url( "/assets/js/library{$min}.js", CODESIGNER ), [ 'jquery' ], time(), false );
		wp_enqueue_style( 'codesigner-library', plugins_url( "/assets/css/library{$min}.css", CODESIGNER ), [ 'dashicons' ], time() );

		$library = Library_Source::get_library_data();

		wp_localize_script( 'codesigner-library', 'CODESIGNER_LIB', [
	...
	public static function register_ajax_actions( Ajax $ajax ) {
		$ajax->register_ajax_action( 'get_wl_library_data', function( $data ) {
			...
			if ( ! empty( $data['editor_post_id'] ) ) {
				...
				if ( ! get_post( $editor_post_id ) ) {
					throw new \Exception( __( 'Post not found.', 'codesigner' ) );
				}
		...
		$ajax->register_ajax_action( 'get_wl_template_data', function( $data ) {
			...
			if ( empty( $data['template_id'] ) ) {
				throw new \Exception( __( 'Template id missing', 'codesigner' ) );
			}

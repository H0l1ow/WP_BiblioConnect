...
class Admin extends Base {
	...
	public function enqueue_scripts() {
		...
		$notice_user_meta = get_user_meta( get_current_user_id(), 'dismissed_admin_notice', true );

		 if ( ( $notice_user_meta != true ) && current_user_can('administrator') ) {

		 	$pointers['_nonce_dismiss_pointer'] = wp_create_nonce('wp-reset_dismiss_notice');

			wp_enqueue_style('wp-pointer');
			wp_enqueue_style( "{$this->slug}-admin-notice", plugins_url( "/assets/css/notice{$min}.css", CODESIGNER ), '', $this->version, 'all' );
			...
			$pointers = [
		    	...
		    	'_wpnonce'		=> wp_create_nonce('admin_notice'),
	...
	public function modal() {
	  ...
	  public function get_pointers() {
	    $pointers = array('target' => '#toplevel_page_codesigner', 'edge' => 'left', 'align' => 'right', 'content' => '<b style="font-weight: 800;">CoDesigner</b> has launched <b style="font-weight: 800;">Modules</b> to empower your WooCommerce Store. Explore our stunning Modules and make your store a powerhouse of sales. <p>    <a  class="cd-notice_ahref" href="' . esc_url(admin_url('admin.php?page=codesigner-modules')) . '"><button >Start Now </button></a>  </p>');
